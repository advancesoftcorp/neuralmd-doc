...
デフォルトでは、Sentinelライセンスをによってライセンス認証が行われます。sannpコマンドのオプションとして、--flexを使用して頂くと、FlexNetライセンスによってライセンス認証が行われます。

力場自動生成（自己学習ハイブリッドモンテカルロ法）機能をご利用の際は、sannp.sh(Windowsの場合はsannp.bat)内のsannpコマンドに、必要に応じて--flexオプションを追記してください。
